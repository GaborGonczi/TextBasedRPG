...
{
	/*std::vector<std::vector<std::string>::iterator>returnValue;
		std::vector<std::string>::iterator i = vec.begin();
		int position = 0;
		while (i != vec.end())
		{
			position = (std::find(pattern.begin() + position, pattern.end(), searched) - pattern.begin());
			if (position != vec.size()) {
				i = vec.begin() + position;
				returnValue.push_back(i);
				position += 1;
			}
			else
...
/*std::vector<int> FileHandler::find(std::string searched, std::vector<std::string>&pattern) {
	std::vector<int>returnValue;
	int position = 0;
	int i = 0;
...
std::vector<std::string> FileHandler::findVector(std::vector<std::string> baseVec, std::string searched1, std::string searched2, std::vector<std::string>::iterator startpos)
{
	std::vector<std::string> returnValue;
	int position1 = 0;
	int position2 = 0;
	if (startpos != pattern.end())
	{
		position1 = (std::find((pattern.begin() + (startpos+1 - pattern.begin())), pattern.end(), searched1) - pattern.begin());
		position2 = (std::find((pattern.begin() + (startpos+1 - pattern.begin())), pattern.end(), searched2) - pattern.begin());
		std::vector<std::string>returnValue((baseVec.begin()+ (position1 + 1)), (baseVec.begin() + (position2)));
